...
//        return ObjectMapper.parseListObjects(repository.findAll(), PersonDTO.class);
    ...
    public PersonDTO update(PersonDTO person) {
        ...
        // Pega o ID que veio no DTO e vai lá no banco resgatar a entidade oficial, usando uma entidade Person.
        // Importante saber
        Person entity = repository.findById(person.getId())
                .orElseThrow(() ->new ResourceNotFoundException("No records found for this ID"));
        // Atualiza os dados no banco através da variavél entity que representa a Entidade.
        // Minha solução: Usaria > mapper.map(person, entity); <
        entity.setFirstName(person.getFirstName());
        entity.setLastName(person.getLastName());
        entity.setAddress(person.getAddress());
        entity.setGender(person.getGender());

        // Salva e transforma para DTO
        var dto = parseObject(repository.save(entity), PersonDTO.class);
        addHateoasLinks(dto);
        return dto;
    }
